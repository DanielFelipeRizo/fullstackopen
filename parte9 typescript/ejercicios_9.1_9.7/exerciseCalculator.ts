...
const parseArrArguments = (args: string[]): number[] => {
  ...
  const arrDays = args.slice(2).map((arg, index) => {
    ...
    if (isNaN(value)) {
      throw new Error(
        `Argument at position ${index + 2} ('${arg}') is not a number`
      );
    }

    if (value > 24) {
      throw new Error(
        `Argument at position ${index + 2} ('${arg}') exceeds 24`
      );
    }

    if (value < 0) {
      throw new Error(
        `Argument at position ${index + 2} ('${arg}') is negative`
      );
    }
...
export const calculator = (arrDays: number[]): exerciseValues => {
